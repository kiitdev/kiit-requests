...
`kiit-requests` depends on `dev.kiit:kiit-inputs` transitively, you don't need to add it separately.
...
| **`Source`** | The protocol a request arrived on: `API`, `CLI`, `Web`, `Queue`, `Bot`, and others, or `Other(name)` for anything not built in. |
...
| **`meta`** | Header-like settings for the request (HTTP headers, CLI flags, queue attributes), as a `Metadata`. |
...
- Depends on `dev.kiit:kiit-inputs` (transitively available to consumers via `api`)
