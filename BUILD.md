# kiit-requests — Build & Publish Guide
...
> **Why this import step matters:** `kiit-requests-kotlin/kiit-requests/build.gradle.kts`'s
...
```bash
./gradlew :kiit-requests:publishAndReleaseToMavenCentral
```
...
1. Bump `libraryVersion` in [`kiit-requests-kotlin/kiit-requests/build.gradle.kts`](./kiit-requests-kotlin/kiit-requests/build.gradle.kts)
...
3. It reads the version via `./gradlew :kiit-requests:printVersion`, verifies a tag for that
...
# Clean build outputs
./gradlew :kiit-requests:clean

# Compile all targets (JVM, Android, iOS)
./gradlew :kiit-requests:build

# Compile only — no tests
./gradlew :kiit-requests:assemble
...
./gradlew :kiit-requests:linkDebugFrameworkIosSimulatorArm64
...
./gradlew :kiit-requests:jvmTest

# All platform tests
./gradlew :kiit-requests:allTests

# Publish to Maven Local (~/.m2) for integration testing against other modules
./gradlew :kiit-requests:publishToMavenLocal
```

Maven Local artifacts are saved to:
```
~/.m2/repository/dev/kiit/kiit-requests/
```
...
[central.sonatype.com/artifact/dev.kiit/kiit-requests](https://central.sonatype.com/artifact/dev.kiit/kiit-requests)
...
```bash
./gradlew :kiit-requests:signKotlinMultiplatformPublication
```
...
`kiit-requests-kotlin/kiit-requests/build.gradle.kts`:
```kotlin
